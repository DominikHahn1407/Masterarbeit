...
class CapsuleNetwork(nn.Module):
    def __init__(
            ...
            hidden_dim=512,
            weight=0.995, # the higher the weight, the more important the image loss gets
            train_on_gpu=True):
    ...
    def forward(self, images, boxes):
        ...
        reconstructions, y, box_output = self.decoder(caps_output, boxes)
        return caps_output, reconstructions, y, box_output
    ...
    def train_model(self, train_loader, criterion, optimizer, n_epochs, print_every=300):
        ...
        for epoch in range(1, n_epochs+1):
            train_loss = 0.0
            self.train()
            for batch_i, (images, target, boxes) in enumerate(train_loader):
                target = torch.eye(self.num_classes).index_select(dim=0, index=target)
                if self.train_on_gpu:
                    images, target, boxes = images.cuda(), target.cuda(), boxes.cuda()
                optimizer.zero_grad()
                caps_output, reconstructions, y, boxes_pred = self.forward(images, boxes)
                image_loss = criterion(caps_output, target, images, reconstructions)
                box_loss = F.mse_loss(boxes_pred, boxes)
            # Überprüfen, ob es sich um eine Bild ohne Krebs handelt (target[:, 1] == 0)
                if torch.all(target[:, 1] == 0):  # Wenn alle Target-Labels 0 sind (kein Krebs)
                    weight1 = 1.0
                    weight2 = 0.0
                else:  # Wenn es sich um Krebs handelt (target[:, 1] == 1)
                    weight1 = self.weight1
                    weight2 = self.weight2
                loss = weight1 * image_loss + weight2 * box_loss # adjust weights
                loss.backward()
                optimizer.step()
                ...
                if batch_i != 0 and batch_i % print_every == 0:
                    ...
                    print('Epoch: {} \tTraining Loss: {:.8f} \tClass loss: {} \tBox loss: {}'.format(epoch, avg_train_loss, image_loss*self.weight1, box_loss*self.weight2))
    ...
    def test_model(self, criterion, test_loader):
        ...
        for batch_i, (images, target, boxes) in enumerate(test_loader):
            target = torch.eye(self.num_classes).index_select(dim=0, index=target)
            ...
            image_loss = criterion(caps_output, target, images, reconstructions)
            box_loss = F.mse_loss(boxes_pred, boxes)
            loss = self.weight1 * image_loss + self.weight2 * box_loss
            test_loss += loss.item()
...
class Decoder(nn.Module):
    def __init__(self, image_size, input_vector_length, hidden_dim, num_classes, train_on_gpu):
        ...
        self.linear_boxes_layers = nn.Sequential(
            nn.Linear(4, 16),                # Größere Eingabegröße für mehr Kapazität
            nn.ReLU(inplace=True),
            nn.BatchNorm1d(16),              # Batch Normalization zur Stabilisierung des Trainings
            nn.Dropout(p=0.3),               # Dropout zur Vermeidung von Overfitting
            nn.Linear(16, 32),               # Größere Anzahl an Neuronen für mehr Ausdruckskraft
            nn.ReLU(inplace=True),
            nn.BatchNorm1d(32),              # Weitere Batch Normalization für den zweiten Layer
            nn.Dropout(p=0.3),               # Weitere Dropout-Schicht
            nn.Linear(32, 16),               # Rückkehr zu einer kleineren Anzahl Neuronen
            nn.ReLU(inplace=True),
            nn.Linear(16, 4),                # Ausgabe der Bounding Box Koordinaten
            nn.Sigmoid()                     # Sigmoid, um Werte im Bereich [0, 1] zu erhalten
        )
        self.num_classes = num_classes
        self.train_on_gpu = train_on_gpu
    ...
    def forward(self, x, boxes):
        ...
        box_output = self.linear_boxes_layers(boxes)
        # y = one hot encoded labels
        return reconstructions, y, box_output
