...
class DICOMCoarseDataset(Dataset):
    def __init__(self, root_dir, num_images_per_class, classes, transform=None):
        # Zufallszahlengenerator initialisieren, um reproduzierbare Stichproben zu gewährleisten
        random.seed(41)

        # Attribute der Klasse setzen
        self.root_dir = root_dir  # Hauptverzeichnis der DICOM-Daten
        self.num_images_per_class = num_images_per_class  # Anzahl der Bilder pro Klasse
        self.classes = classes  # Liste der Klassennamen (z. B. ["Nodule", "Non-Nodule"])
        self.transform = transform  # Transformationen, die auf die Bilder angewendet werden sollen
        self.image_paths = []  # Liste für die Pfade der ausgewählten Bilder
        self.labels = []  # Liste für die zugehörigen Klassenlabels

        # Berechnung der Anzahl der Bilder in der Klasse "Nodule"
        temp_folder = os.path.join(root_dir, "Nodule")  # Pfad zum "Nodule"-Ordner
        self.num_images_per_class = len([f for f in os.listdir(temp_folder) if f.endswith('.dcm')])

        # Iteration über alle Klassen und ihre Namen
        for class_label, class_name in enumerate(self.classes):
            # Pfad zum aktuellen Klassenordner
            class_folder = os.path.join(root_dir, class_name)
            
            # Prüfen, ob der Ordner existiert
            if os.path.isdir(class_folder):
                # Alle DICOM-Dateien (.dcm) im Ordner auflisten
                dicom_files = [f for f in os.listdir(class_folder) if f.endswith('.dcm')]

                # Wenn genügend Dateien vorhanden sind, wähle eine Zufallsstichprobe
                if len(dicom_files) >= self.num_images_per_class:
                    selected_files = random.sample(dicom_files, self.num_images_per_class)
                else:
                    # Andernfalls nimm alle verfügbaren Dateien
                    selected_files = dicom_files

                # Füge die ausgewählten Dateien und ihre Labels zu den Listen hinzu
                for file_name in selected_files:
                    self.image_paths.append(os.path.join(class_folder, file_name))  # Vollständiger Dateipfad
                    self.labels.append(class_label)  # Klassenlabel entsprechend der Reihenfolge in `self.classes`
...
class DICOMPETDataset(Dataset):
    ...
    def __getitem__(self, index):
        img_path = self.data.iloc[index]['File Path']
        dicom_image = pydicom.dcmread(img_path)
        image = dicom_image.pixel_array
        image = Image.fromarray(np.uint8(image))
        if self.transform:
            image = self.transform(image)
        label = self.data.iloc[index]['Label']  # Hier ist das Label als String (z.B. 'cat')
    ...
    def visualize_images(self, num_images=5):
        ...
        _, axes = plt.subplots(1, num_images, figsize=(25,25))
        if num_images == 1:
            axes = [axes]
        for i in range(num_images):
            ...
            if isinstance(image, torch.Tensor):
                image = image.squeeze().numpy()
            axes[i].imshow(image, cmap="gray")
            axes[i].set_title(f"Label: {self.class_names[label]}")
            axes[i].axis("off")
        plt.show()


class DICOMPETFineDataset(Dataset):
